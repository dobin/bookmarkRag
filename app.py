...
def _graphrag_status(notebook: str) -> dict | None:
    ...
    output_dir = _BASE_DIR / "grag" / notebook / "output"
    required_files = (
        "documents.parquet",
        "text_units.parquet",
        "entities.parquet",
    ...
    vector_store_exists = (output_dir / "lancedb").is_dir()
...
def search(notebook: str):
    ...
    if request.method == "POST":
        ...
        if query:
            ...
            for entry in _load_bookmarks(notebook):
            ...
            if search_dir.exists():
                ...
                for filepath in sorted(search_dir.glob(pattern)):
                    ...
                    content_exists = (_input_dir(notebook) / content_filename).is_file()
                    summary_exists = (
                        _summaries_dir(notebook) / f"{Path(content_filename).stem}.llm"
                    ).is_file()
